...
void init_kmalloc(multiboot_info_t *mbi)
{
...
void *krealloc(void *source, size_t new_size)
{
	/* Sanity Check */
	 ...
	 * this is for security reasons, so that we dont just read the whole memory
	 *to the user then again this is not a level seperated kernel so lol@me
	 **/
	if (region) {
...
static block_header_t *__get_block_header(void *buffer)
{
	block_header_t *current		  = __store_global_head;
...
static block_header_t *__splice_block_out(
	block_header_t *lo_header, block_header_t *hi_header)
{
	// change size
	lo_header->size += hi_header->size + BLK_HEADER_SIZE;

	// the high header could be the last block in the store, handle it in the
	// edge case
	if (hi_header->next) {
		// splice pointers
...
static block_header_t *__splice_block_in(
	block_header_t *current, size_t alloc_size)
{
