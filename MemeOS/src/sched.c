...
tid_t sched_register_thread(void *(*callable)(void *), void *arg)
{
...
void schedule(void)
{
	...
	if (__current_task->status != NEW)
		...
		/* task switching is just infinite recurison over all the running
		 * threads */
#ifdef THREAD_DUMMY_CONTEXT_START
	...
	if (__current_task->status == NEW) {
		...
		__current_task->status = RUNNING;
		START_NEW_THREAD(__current_task, sched_finalize_thread);
	} else {
		__current_task->status = RUNNING;
		/* task switching is just infinite recurison over all the running
		 * threads */
...
void __sleep_on(unsigned long systicks)
{
	kthread_mutex_lock(&__global_sched_lock);
...
static void splice_inq(task_struct_t **head_ptr, task_struct_t *element)
{
	if (*head_ptr) {
...
static void splice_outq(task_struct_t **head_ptr, task_struct_t *element)
{
