...
void load_idt(void) { __asm__ volatile("lidt (idtr)"); }
...
void die(char *msg, unsigned long esp_ptr, long error_code)
{
...
void do_debug(unsigned long esp, long error_code)
{
	die("Debug", esp, error_code);
}


void do_nmi(unsigned long esp, long error_code) { die("NMI", esp, error_code); }
...
void do_overflow(unsigned long esp, long error_code)
{
	die("Overflow\n", esp, error_code);
}


void do_bounds(unsigned long esp, long error_code)
{
	die("Bounds Range Exceeded", esp, error_code);
}


void do_invalid_op(unsigned long esp, long error_code)
{
	die("Invalid Operand", esp, error_code);
}


void do_device_unavailable(unsigned long esp, long error_code)
{
	die("Device Not Available\n", esp, error_code);
}


void do_double_fault(unsigned long esp, long error_code)
{
	die("DOUBLE FAULT", esp, error_code);
}


void do_coprocessor_segment_overrun(unsigned long esp, long error_code)
{
	die("Coprocessor Segment Overrun", esp, error_code);
}


void do_invalid_tss(unsigned long esp, long error_code)
{
	die("Invalid TSS", esp, error_code);
}

void do_segment_not_present(unsigned long esp, long error_code)
{
	die("segment not present", esp, error_code);
}


void do_stack_segment_fault(unsigned long esp, long error_code)
{
	die("stack segment", esp, error_code);
}


void do_general_protection_fault(unsigned long esp, long error_code)
{
	die("General Protection Fault", esp, error_code);
}


void do_page_fault(unsigned long esp, long error_code)
{
	die("Page Fault", esp, error_code);
}


void do_reserved(unsigned long esp, long error_code)
{
	die("Reserved", esp, error_code);
}


void do_x87_fpu_except(unsigned long esp, long error_code)
{
