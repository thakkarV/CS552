...
#define UFS_NUM_MAX_BLOCKS                                                     \
	(UFS_TOTAL_NUM_BLOCKS - UFS_SIZEOF_INODE_ARRAY - UFS_NUM_BITMAP_BLOCKS - 1)
...
// max length of the name for any file (dir or reg) inclusive of the terminal
// null byte
...
/* LOGICAL LAYOUT OF UNIX FILESYSTEM - 256 byte block size
	...
	II) Index Nodes Array
		...
		Each inode is 64 bytes in size. layout of the node is described in the
   struct code

	III) Block Bitmap
		After index blocks are 4 blocks of block bitmaps used to track allocated
   and unallocated blocks in the fs
...
typedef struct inode_t {
	...
	/* inode_t layout
	 +------+------+---+---+---+----+---+----+----+---+---+---+----+---+--+
	 | Type | Size | 8x BlkPtr | Indirect BlkPtr | Double Indirect BlkPtr |
	 +------+------+---+---+---+----+---+----+----+---+---+---+----+---+--+
	**/
	// clang-format on
	inode_status_t type;
