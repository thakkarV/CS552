...
init_kmalloc(multiboot_info_t * mbi)
{
	...
	#ifdef DEBUG
		printf("malloc base addr = 0x%x\n", __mem_store_base);
		printf("malloc list length = 0x%x\n", __mem_store_size);
		printf("malloc end addr = 0x%x\n", __mem_store_end);
	#endif
...
kmalloc(size_t size)
{
	...
	**/
	if (current->size >= alloc_size + 2 * BLK_HEADER_SIZE)
	{
		...
		block_header_t * new = __splice_block_in(current, alloc_size);

		#ifdef DEBUG
			printf("new block header added addr = 0x%x | size = 0x%x\n", new, new->size);
			printf("with prev = 0x%x and next = 0x%x\n", new->prev, new->next);
		#endif
	}	
...
kfree(void * buffer)
{
	...
	#ifdef DEBUG
		printf("freeing block with data address = 0x%x\n", buffer);
		printf("freeing memory header = 0x%x\n", header);
		printf("Header region size = 0x%x\n", header->size);
	#endif
